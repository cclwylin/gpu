...
- **Phase-1 LT** — `ResolveUnitLt` in [`systemc/blocks/resolveunit/`](../../systemc/blocks/resolveunit/) (Sprint 34). Wraps `gpu::pipeline::resolve`.
- **Phase-2 CA** — `ResolveUnitCa` (Sprint 26). 1 cyc/pixel placeholder when MSAA on; pass-through (zero stamp) when MSAA off, to avoid wall-clock stalls in 1× scenes.
- **sw_ref** — `gpu::pipeline::resolve` (Sprint 3). Box filter `(s0+s1+s2+s3+2) >> 2` per channel.
- **Out of scope for v1**: `glBlitFramebuffer` explicit-resolve path, depth / stencil resolve, weighted filters, FBO-base unaligned write.
...
- [x] 1× fast path:**share** (sw_ref `gpu::pipeline::resolve` is a no-op when MSAA off; CA charges zero cycles).
- [x] Depth / stencil resolve:**out of v1** (semantics unclear; spec leaves it undefined).
- [x] Weighted filter:**out of v1**, fixed box.
- [ ] FBO base unaligned — Phase 2.x; current models write directly into the host-side `fb.color` vector.
